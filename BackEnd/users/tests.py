...
class EditTest(TestCase):
    ...
    # Errors change user
    def test_post_edit_change_username_error(self):
        self.client.login(username=self.userData['username'],
                          password=self.userData['password'])
        credentials = {'username': ' ', 'password1': '1234x567', 'password2': '1234x567', 'email': 'zekaqui@mail.com'}
        response = self.client.post('/users/config/', credentials, follow=True)
        self.assertNotEqual(response.context['user'].username, credentials['username'])

    def test_post_edit_change_email_error(self):
        self.client.login(username=self.userData['username'],
                          password=self.userData['password'])
        credentials = {'username': 'zekaqui', 'password1': '1234x567', 'password2': '1234x567',
                       'email': 'zekaquimail.com'}
        response = self.client.post('/users/config/', credentials, follow=True)
        self.assertNotEqual(response.context['user'].email, self.credentials['email'])

    def test_post_edit_change_pwd_error(self):
        self.client.login(username=self.userData['username'],
                          password=self.userData['password'])
        credentials = {'username': 'zekaqui', 'password1': '1234x590', 'password2': ' ', 'email': 'zekaqui@mail.com'}
        self.client.post('/users/config/', credentials, follow=True)
        user = User.objects.get(username=self.credentials['username'])
        self.assertNotEqual(user.check_password(credentials['password1']), True)
